...
>
> ## ✅ 2026-07-26：§3 的 DDS 投递之谜已结案 —— 真凶是 `net.core.rmem_max`
> 不是"独立进程的 DDS 参与者投递不到栈内节点"这种架构性问题。JP4 出厂 `/etc/sysctl.conf` 设 `net.core.rmem_max=26214000`（26MB），移植时整条丢了，系统停在内核默认 `212992`（208KB）——**凡是单帧超过 208KB 的话题就几乎一帧都过不去**，848x480 Z16 深度帧正好是 814KB。
> 补回 26MB 之后实测：一个**完全独立的 systemd unit**（`d455-camera.service`）起的相机节点与栈**同图可见（31 个节点）**，depth/infra1/infra2 三路 848x480 全部 30Hz 满速，IMU 197.6Hz。
> **所以 §3 里"把桥塞进栈 launch"的整改方向是不必要的**，独立 unit 就是对的。详见 `PORT-STATUS.md` 的 2026-07-26 条目。
