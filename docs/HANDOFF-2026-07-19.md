...
## Phase 4 之前：JP4 侧还欠一晚（机主在场，三件事同一晚搭车做完）
- **走路测试**（§7 验收补账——缩盘手术后一直没真走过，等 Phase 4/5 之后再发现异常
  就分不清锅是谁的了）。
- **MCU 使能序列捕获**：`tools/mcu-capture/start-capture.sh`（目前只捕
  udev/dmesg/lsusb，还需补 TCA6424 GPIO 状态读取）。站立触发正好是捕获时机，
  跟走路测试天然搭车。
- **BT 固件拷出**：从 JP4 侧 /lib/firmware 拷出 rtl8821cu_fw / rtl8821cu_config
  （见妥协点 8）。
...
⚠️ **先记住一个陷阱**：eMMC 上现存的 `LABEL jp5` 是 2026-07-11 的 proof 模式占位——
LINUX/FDT 是 JP4 内核的字节拷贝、INITRD 指向 stock initrd、`root=/dev/nvme0n1p1`。
**绝不能只把新文件放进 /boot-jp5/ 就切换**（那会用 5.10 内核 + JP4 initrd 去 rw
挂载 JP4 保底系统的根分区）。必须整段重写 stanza（LINUX/FDT/INITRD 全指
`/boot-jp5/`、`root=/dev/nvme0n1p2`、APPEND 加 `panic=15`），然后**重新生成 pivot
上的三份 `*-saved` 副本**——钩子回退时复制的就是 jp4-saved。同一个坑的另一半：
狗上现存的三份 `*-saved` 也是 07-11 的老货（rescue stanza 还没吃到 07-17 的
LINUX 行修复，直接拿它们恢复会静默弄坏逃生舱），**③重新生成之前一律不可信**。
存档基准：仓库 `docs/extlinux-live-2026-07-19.conf` 和
`docs/dtb-live-jp4-2026-07.dts.gz` 是 2026-07-19 抓的线上 ground truth。
