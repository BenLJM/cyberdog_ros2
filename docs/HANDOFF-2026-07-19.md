...
⚠️ **唯一遗留 = Wi-Fi/相机/GPS/RealSense 全线 -517**（外设没通电）。根因已精确
定位:**TCA6424 GPIO 扩展器(0x22/0x23 @ i2c@3160000)在 5.10 上 I2C probe 失败
-121**,连锁使十几条 GPIO 门控电源轨永久延迟。这是 **Phase 5(外设点亮)**级别的
多轮内核/DT 调试,不是 Phase 4 的收尾。修复方向 + 调试桥(Mac→笔记本 10.0.0.176
→USB→192.168.55.1)全部见 `docs/PHASE4_BOOT_RESULTS.md`。
