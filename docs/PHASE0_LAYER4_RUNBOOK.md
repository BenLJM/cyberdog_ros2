...
- [~] **jp5-mirror: r35.6.4 trio ✓ + r32.5.2 pair ✓ + cp38 wheels ✓ + 14 repo mirrors ✓** — on SSD, SHA256-verified. **apt-repo snapshot of `repo.download.nvidia.com/jetson/{common,t194}/dists/r35.6`: fetch STARTED 2026-07-11 on-dog** (`tools/fetch-phase2-gaps.sh`, log `fetch-phase2-gaps.log`; excludes `deepstream*`/`nsight*` — logged, re-run to resume). The earlier "defer to Phase 1" call was reversed by the 2026-07-11 review: post-EOL apt-repo availability is exactly the risk class this mirror exists for. **Same fetch also adds the Phase-8 voice stack** (whisper.cpp, kokoro-onnx + model files, kokoro-tts, openWakeWord, ggml-small weights) — previously unmirrored. **Re-run `replicate-to-ssd.sh` next time the SSD is attached.**
- [x] **Layer 4b BL payloads copied to SSD, SHA256SUMS validated ✓**
- [x] **Layer 3b QSPI/boot0/1 dumps replicated to SSD, SHA256SUMS validated ✓** (§4c). *(Note 2026-07-11: `emmc-boot0/boot1.img` are all-zeros — genuine, not a capture bug: Xavier NX boots from QSPI-NOR; the 32 MiB QSPI dump carries the real bootloader chain.)*
...
- [ ] MCU power-gating capture on JP4 side (review D7) — **needs owner** to trigger motion (harness: `tools/mcu-capture/start-capture.sh`; do before Phase 2 surgery while JP4 is the daily driver)
- [x] Tag `v0.1-phase0-complete` created + pushed (2026-07-11)
- [x] **Then Phase 0.5 (boot-path disambiguation — review D2) before any Phase 2 work** — **COMPLETE 2026-07-11** (commit ab817b5; all 4 questions answered, see `PHASE0_BOOT_MECHANISM_FINDINGS.md` v2)

## 8. What's next (Phase 1 preview)

Phase 1 (x86 host dev environment) — `JETPACK5_HUMBLE_PORT_PLAN.md` §9 — sets up the cross-build toolchain, ROS 2 Humble x86 dev install, and r35.6.4 BSP source tree alongside the r32.5.2 one. None of it touches the dog. Expect ~3 evenings.

The first thing Phase 1 actually does is rebuild the same r32.5.2 BSP with `flash.sh --no-flash` to confirm the host can drive Tegra flashing end-to-end *before* introducing the much larger r35.6.4 source tree. That's the cheapest possible smoke-test of the host.
