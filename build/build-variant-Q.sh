#!/bin/bash
# =============================================================================
#  变体 Q —— P + Stage-12：加 debugfs 读 NVCSI 接收端寄存器
#
#  P 之后链路每一环都验证正确却仍零帧，而唯一的接收端证据(RCE ftrace 三计数)
#  本身可疑(error_config 被 stage3 置零 + vinotify 的 trace 格式 R32 未必按
#  R35 发)。CONFIG_STRICT_DEVMEM=y 又挡住了用户态读寄存器 ⇒ 加 debugfs。
#  纯只读诊断，不改任何行为。
#
...
OUT=/tmp/kb
DEST=/work/nvcsi-variants/Q-regdump
...
python3 /work/stage12-nvcsi-regdump.py
