#!/bin/bash
# =============================================================================
#  变体 S —— P + Stage-14：在 argus 真正走的路上打印包匹配条件
#
#  stage13 挂在 vi5_setup_surface 上,实测 r32-vi5 计数为 0 ⇒ stage4/13 在
#  argus 路径下也是死代码(argus 自己在用户态填整个描述符)。这已是第三次踩
#  '改了没被走到的路径'。stage14 改挂 fusa-capture 的 vi_capture_request
#  (ftrace 实测调用 9 次,stage5 的 r32-sync 日志也确实出现过)。
#
...
OUT=/tmp/kb
DEST=/work/nvcsi-variants/S-match
...
python3 /work/stage14-match-dump-argus.py
...
for s in r32_camera_power r32-stage2 r32-csi5 r32-vi5 r32-sync r32-mipical r32-mipisoc r32-settle r32-csistart r32-match tegra194_nvcsi_r32_start_streams tegra_camrtc_r32_camera_power_enabled nvcsi-t194-prod; do
