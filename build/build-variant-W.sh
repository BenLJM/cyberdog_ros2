#!/bin/bash
# =============================================================================
#  变体 W —— V + Stage-17：让 RCE 自己 dump NVCSI PHY 寄存器
#
#  V 证实 RCE 接受了全部 CSI 配置(result=0)但 NVCSI 中断仍 Δ=0。要往下查得看
#  PHY/CIL 链路状态寄存器,而 T194 的 NVCSI 寄存器映射不在本树里。R32 消息集
#  自带 CAPTURE_PHY_STREAM_DUMPREGS_REQ(0x3C) —— 应答只回 result,寄存器内容
#  由固件打进自己的日志,经 rtcpu_string 就能读到。既不用寄存器映射,
#  也不用在内核里 mmap 任何东西。
#
#  目前最大的未验证假设: stage3 的三条 CSI 消息是 fire-and-forget,
#  rc=264 只是 IVC 写入字节数。且 channel_id 硬编码成 64+1=65,而 capture-ivc
#  要求先 register_control_cb 申请 trans_id 才能路由应答 —— 65 号槽没有我们
#  的回调,RCE 的应答被丢(甚至可能串到别的客户端)。V 改成正规流程,
...
OUT=/tmp/kb
DEST=/work/nvcsi-variants/W-dumpregs
...
python3 /work/stage16-csi-resp.py
python3 /work/stage17-phy-dumpregs.py
...
for s in r32_camera_power r32-stage2 r32-csi5 r32-vi5 r32-sync r32-mipical r32-mipisoc r32-settle r32-csistart r32-resp PHY_DUMPREGS tegra194_nvcsi_r32_start_streams tegra_camrtc_r32_camera_power_enabled nvcsi-t194-prod; do
