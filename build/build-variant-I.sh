...
OUT=/tmp/kb
DEST=/work/nvcsi-variants/I-stage4
...
for probe in "camera-power.dtsi:$DTS" "camrtc_device_group_busy:$NVID/drivers/platform/tegra/rtcpu/device-group.c" \
             ...
             "r32-vi5:$NVID/drivers/media/platform/tegra/camera/vi/vi5_fops.c"; do
    pat="${probe%%:*}"; f="${probe#*:}"
    n=$(grep -c "$pat" "$f" || true)
...
cleanup() {
    ...
    echo "  残留检查: dts=$(grep -c camera-power.dtsi "$DTS" || true) stage2=$(grep -c r32-stage2 "$NVID/drivers/video/tegra/host/nvcsi/nvcsi-t194.c" || true) csi5=$(grep -c r32-csi5 "$NVID/drivers/media/platform/tegra/camera/nvcsi/csi5_fops.c" || true) vi5=$(grep -c r32-vi5 "$NVID/drivers/media/platform/tegra/camera/vi/vi5_fops.c" || true) gate=$(grep -c r32_camera_power "$NVID/drivers/platform/tegra/tegra-camera-rtcpu.c" || true)（都须 0）"
}
trap cleanup EXIT
...
for s in r32_camera_power r32-stage2 r32-csi5 r32-vi5 tegra_camrtc_r32_camera_power_enabled nvcsi-t194-prod; do
