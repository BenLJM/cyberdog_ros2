#!/bin/bash
# =============================================================================
#  变体 X —— V + Stage-18：OPEN 之前先发 PHY_STREAM_RESET
#
#  软件侧唯一还没试过的东西。CAPTURE_PHY_STREAM_RESET_REQ(0x3A) 两代消息集
#  里都有,但上游内核从不使用(R35 的 RCE 自己管 PHY 复位)。现状是配置全被
#  接受(result=0,且与 DUMPREGS 的 result=1 形成对照)却收不到数据,正对应
#  硬件没进入接收状态 —— 一次显式 PHY 复位对症。返回码本身就是信息。
#
...
OUT=/tmp/kb
DEST=/work/nvcsi-variants/X-phyreset
...
python3 /work/stage18-phy-reset.py
...
for s in r32_camera_power r32-stage2 r32-csi5 r32-vi5 r32-sync r32-mipical r32-mipisoc r32-settle r32-csistart r32-resp PHY_RESET tegra194_nvcsi_r32_start_streams tegra_camrtc_r32_camera_power_enabled nvcsi-t194-prod; do
