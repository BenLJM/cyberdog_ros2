#!/bin/bash
# =============================================================================
#  变体 AE —— AD + Stage-25：CSI 端口消息按端口位图幂等
#
#  AD 的 once 只盖住批量入口。v4l2 每通道 STREAMON 的 per-channel 入口
#  (csi5_start_streaming→set_config_r32/open_r32)仍会对已开端口重复发消息,
#  实测三路并发竞态: 主摄+鱼眼A 满速 29.8fps 而鱼眼B 0 帧零错误
#  (port0/1 同一 CIL brick,重复 SET_CONFIG 扰动邻端口)。
#  Stage-25: 两张原子位图,每端口每种消息全局只发一次,随 RCE resume 清零。
#
...
OUT=/tmp/kb
DEST=/work/nvcsi-variants/AE-perport
...
python3 /work/stage25-perport.py
...
for s in r32_camera_power r32-stage2 r32-csi5 r32-vi5 r32-sync r32-mipical r32-mipisoc r32-settle r32-csistart r32-padcfg r32-recal r32-csipwr r32-vipwr r32-once r32-perport tegra194_nvcsi_r32_start_streams tegra_camrtc_r32_camera_power_enabled nvcsi-t194-prod; do
