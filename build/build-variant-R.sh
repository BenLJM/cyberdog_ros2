#!/bin/bash
# =============================================================================
#  变体 R —— P + Stage-13：打印 VI 通道的包匹配条件(ch_cfg.match)
#
#  发射侧已彻底验证通过(I2C 直读 chip_id/分辨率/HTS/streaming 全对)，
#  RCE trace 解码也验证正常(未知类型事件 0 ⇒ 三计数全 0 是真的)。
#  接收侧只剩一个没测过的量: argus 填的 match.stream/vc/datatype。
#  ⚠️ 不含 stage12(它把 debugfs_create_file 挂在 finalize_poweron，引入 VI 回归)。
#
...
OUT=/tmp/kb
DEST=/work/nvcsi-variants/R-match
...
python3 /work/stage13-match-dump.py
