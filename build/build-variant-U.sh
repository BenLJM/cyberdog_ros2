#!/bin/bash
# =============================================================================
#  变体 U —— T + Stage-15：NVCSI 接收端寄存器 debugfs(重做版)
#
#  0004 让 nvcsilp 起来了(enable_cnt 0→2)但仍零帧。各层都已验证正确或排除,
#  只剩一个没拿到一手证据的问题: 接收端在物理层到底有没有看到跳变。
#  stage12 想法对、挂点错(挂在被握手计时的 finalize_poweron,引入 VI 回归);
#  stage15 改挂 late_probe 这条冷路径。
#
...
OUT=/tmp/kb
DEST=/work/nvcsi-variants/U-regdump
...
python3 /work/stage15-regdump-probe.py
...
for s in r32_camera_power r32-stage2 r32-csi5 r32-vi5 r32-sync r32-mipical r32-mipisoc r32-settle r32-csistart r32-nvcsi-regdump tegra194_nvcsi_r32_start_streams tegra_camrtc_r32_camera_power_enabled nvcsi-t194-prod; do
