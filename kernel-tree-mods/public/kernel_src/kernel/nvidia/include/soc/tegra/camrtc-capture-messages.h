...
/*
 * ---------------------------------------------------------------------
 * CyberDog r32 capture-ABI switch (CONFIG_TEGRA_CAPTURE_R32_ABI)
 *
 * The RCE camera firmware on this board lives in eMMC boot0 and is the
 * L4T r32 (JetPack 4) build; it cannot be reflashed.  Its capture ABI is
 * not the r35 one -- most visibly CAPTURE_CHANNEL_SETUP_REQ is 0x10 in
 * r32 and 0x1E in r35 (r35 renamed 0x10 to CAPTURE_CONTROL_RESERVED_10),
 * and struct capture_channel_config grew 216 -> 272 bytes.
 *
 * When the option is enabled the whole kernel build sees the r32 layout,
 * so every translation unit agrees -- there is no ODR hazard.  When it is
 * disabled this file preprocesses byte-for-byte as shipped by NVIDIA.
 * ---------------------------------------------------------------------
 */
#if defined(__KERNEL__)
#include <linux/kconfig.h>
#if IS_ENABLED(CONFIG_TEGRA_CAPTURE_R32_ABI)
#include "camrtc-capture-messages-r32.h"
#define CAMRTC_CAPTURE_MESSAGES_R32_ABI_SELECTED 1
#endif
#endif

#ifndef CAMRTC_CAPTURE_MESSAGES_R32_ABI_SELECTED
...
#endif /* !CAMRTC_CAPTURE_MESSAGES_R32_ABI_SELECTED */
