...
/*
 * camrtc-capture-r32.h
 *
 * L4T r32 (JetPack 4) RCE capture firmware ABI.
 *
 * VERBATIM COPY of r32 kernel/nvidia/include/soc/tegra/camrtc-capture.h
 * (CyberDog factory kernel, mirror branch "athena"), with only these
 * mechanical edits:
 *   - include guard renamed  INCLUDE_CAMRTC_CAPTURE_H -> ..._R32_H
 *   - a compatibility appendix at the tail that re-declares the handful of
 *     structs that exist ONLY in r35 and that r35 kernel code still
 *     references from code paths which are compiled out (or made inert)
 *     when CONFIG_TEGRA_CAPTURE_R32_ABI=y.
 *
 * This file is the wire format spoken by the r32 RCE firmware that lives in
 * the CyberDog eMMC boot0 partition.  DO NOT "modernise" it.
 */
/*
...
#ifndef INCLUDE_CAMRTC_CAPTURE_R32_H
#define INCLUDE_CAMRTC_CAPTURE_R32_H
...
/*
 * =====================================================================
 * r35 compatibility appendix -- NOT part of the r32 firmware ABI.
 *
 * These declarations exist only so that r35 kernel sources that were
 * written against the r35 "buffer-table + memoryinfo" per-frame model keep
 * compiling when CONFIG_TEGRA_CAPTURE_R32_ABI=y.  Nothing in this appendix
 * is ever handed to the r32 RCE firmware: the r32 struct
 * capture_channel_config / capture_channel_isp_config have no
 * requests_memoryinfo field, so the firmware is never told where these
 * buffers are.  The kernel keeps filling them (they double as the pin
 * bookkeeping for capture_common_pin_and_get_iova()), and the r32 reloc
 * pass writes the real IOVAs into the capture descriptor itself, which is
 * what r32 firmware actually reads.
 * =====================================================================
 */

/* r35 spells these without the leading underscores. */
#ifndef CAPTURE_IVC_ALIGN
#define CAPTURE_IVC_ALIGN		__CAPTURE_IVC_ALIGN
#endif
#ifndef CAPTURE_DESCRIPTOR_ALIGN
#define CAPTURE_DESCRIPTOR_ALIGN	__CAPTURE_DESCRIPTOR_ALIGN
#endif

/*
 * VI unit identifiers.  r32 has a single VI unit on T194 and therefore never
 * put a vi_unit_id in the wire config; these are kept purely so that r35
 * kernel-side bookkeeping (vi_instance_table[], vi_pdevices[]) compiles.
 */
#ifndef VI_UNIT_VI
#define VI_UNIT_VI	U32_C(0x0000)
#endif
#ifndef VI_UNIT_VI2
#define VI_UNIT_VI2	U32_C(0x0001)
#endif
...
#endif /* INCLUDE_CAMRTC_CAPTURE_R32_H */
