...
static int therm_fan_est_probe(struct platform_device *pdev)
{
	...
	/*
	 * CyberDog JP5 port (2026-07-25): Linux 5.10 introduced an explicit
	 * enable step -- a zone registered without it stays in
	 * THERMAL_DEVICE_DISABLED, so its governor never runs and every bound
	 * cooling device is pinned at cur_state 0.  NVIDIA's R35 tree never
	 * added the call, which left thermal_zone5 (thermal_fan_est) disabled
	 * on this board: the kernel never touched the fan at all and cooling
	 * depended entirely on a userspace daemon.  Enable it here.
	 *
	 * Non-fatal on failure: a disabled zone is exactly the pre-patch
	 * behaviour, and the userspace fan daemon still covers it.
	 */
	err = thermal_zone_device_enable(est_data->thz);
	if (err) {
		pr_err("THERMAL EST: thz enable failed (%d), zone stays disabled\n",
		       err);
		err = 0;
	}
