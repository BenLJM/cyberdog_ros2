...
## 🧪 JP4 在线对照实验（自恢复设计，安全跑通）

之前判断「开进 JP4 需要机主在场」——**其实可以做成自恢复的**，思路与 `jp5-exp` 相同：

> **开机第一件事就是把 extlinux 的 `DEFAULT` 拨回 `jp5`**，之后才采数据。
> 这样即使后面任何环节失败或挂死，一次断电就回到 JP5，不需要人工改配置。

实现：往 JP4 rootfs 装一个 `jp4-cam-baseline.service`（oneshot，`After=local-fs.target`），
脚本先拨回 DEFAULT → 采基线 → 触发一次 `START_LIVE_STREAM` → 写日志 → 自动重启。
**实测完美工作**：JP4 正常启动 → 采集 → 自动回到 JP5，全程无人工干预。
（服务用后已撤除自启链接。）

### 结果一：JP4 确实工作、JP5 确实不工作（对照成立）

JP4 采集期间的 dmesg **没有任何采集错误** —— 只有 RCE 的两条正常信息
（ISP error-notification 警告 + `vi5_hwinit`）；而 JP5 每次都有
`vi capture get status failed`。

### 结果二：🔴 推翻了一条关键"证据"

**JP4（相机正常工作）的 `tegra_rtcpu` trace 同样只有 6 条 `rtcpu_string`，
`vinotify_event` / `nvcsi_intr` **也是 0**。**

⇒ 「三计数全 0」**根本不是失败的证据** —— R32 固件就是不发这类 trace 记录。
之前据此得出的「NVCSI 在链路层完全静默」结论**证据不成立**，需要撤回。
（`camrtc/irqstat` 那条独立证据仍在，但 JP4 的 4.9 内核没有这个接口，无法对照。）

### 结果三：取流时的时钟状态**逐值相同**

| 时钟 | JP4（工作） | JP5（不工作） |
|---|---|---|
| `nvcsi` | 72,461,538 | **72,461,538** |
| `nvcsilp` | 204,000,000 | **204,000,000** |
| `vi` | 115,200,000 | **115,200,000** |
| `isp` | 281,600,000 | **281,600,000** |
| `ve` / `ispa` powergate | 1 / 1 | 1 / 1 |

（我此前看到 JP5 的 `nvcsi` 是 314 MHz 并判定"= BPMP 上限、正确"——
那是**空闲态**的读数；取流时 JP5 同样会降到 72.46 MHz。两边完全一致。）

⇒ **时钟 / 电源域这一整类假设排除。**

### 结果四：唯一还剩的可见差异

`cam RTCPU IRQ`：JP4 = **65**，JP5 = **46**。
多半只是两代内核 virq 编号不同（同一条 GIC 中断，Linux 侧虚拟号不同），
但**尚未证实**。子设备绑定两边都完整（`nvcsi--1/2/3` + 三个传感器全部 bound）。
...
### 帧率攻坚：4.4 → 30.1 fps（达到出厂帧率）

机主要求「帧率先达到原版」。出厂标定文件
`athena_tracking/config/camera_AI.yaml` 给出原版规格：**1280×960**，MEI 全向模型，
`gamma1=674.669 gamma2=682.214 u0=646.499 v0=497.005 xi=0.176364`。目标帧率 = **30 fps**。

**四步各自的贡献（每一步都先计时再改）**

| 改动 | 结果 |
|---|---|
| ① `Image.data` 给 `array.array('B')`（原来给 `bytes` → Foxy 逐元素断言） | 3.1 → 19.5 fps |
| ② 按墙钟节流替代按帧计数 | 稳定在目标值附近 |
| ③ **LUT 查表**替代 float 流水线（65536 项表，一次 fancy-index 出结果） | 转换 28ms/通道 → 11ms/**三通道** |
| ④ **节流阈值留容差**（0.8×周期，不能等于 1/fps） | 17.7 → **30.1 fps** |

④ 是最反直觉的一条：阈值正好等于源周期时，处理完一帧阻塞等下一帧，醒来时
`now - t_last_pub` 常常差几十微秒不到一个周期 → 这一帧被跳过 → 下次要等两个周期
→ **帧率正好砍半**。实测卡在 17.7 ≈ 30/2，而处理只用 25ms 本该跑满。

另外去掉了 rclpy 的定时器 + `spin`（本节点没有任何订阅，走执行器纯属白担调度开销），
改成自己 `select` 阻塞的裸循环；复用 `Image`/`CameraInfo` 消息对象；
`camera_info` 降到 1 Hz（内参不随帧变）。

**验收**：服务态 `取 30.3 fps / 发 30.1 fps`（每帧：排空 7.5ms + 转换 11.3ms + 发布 14.1ms）；
独立订阅者 **20 秒收到 600 帧 = 29.73 fps**；CPU 71% 单核（六核机型 ~12%，
`Nice=5`/`CPUWeight=50` 让路给运动栈）；0 failed。

### 分辨率 ↔ 帧率的取舍（publish ≈ 30 MB/s 是硬墙）

| BIN | 输出 | 转换 | 发布 | 实测帧率 |
|---|---|---|---|---|
| **4（默认）** | **526×390** | 11.3ms | 14.1ms | **30.1 fps** |
| 3 | 701×520 | 19.1ms | 33.0ms | 19.1 fps |
| 2 | 1052×780 | 30.7ms | 76.5ms | 9.3 fps |

`AI_CAM_QOS=best_effort` 实测**没有帮助**（BIN=3 下 34.1ms vs reliable 33.1ms）——
开销在 rclpy 序列化 + 环回 UDP 分片，不在 DDS 可靠性上。
⇒ 想同时要出厂的 1280×960 和 30fps，Python 做不到（光 publish 就要 ~110ms/帧），
需要把节点改写成 C++。

### 已知边界

- **`camera_info` 是针孔近似**：出厂是 MEI 全向模型（带 `xi` 镜面参数），
  ROS 的 `CameraInfo` 没有这个模型。这里放的是把出厂焦距/主点按分辨率线性缩放
  的结果（实测 fx=277.2 = 674.669×526/1280 ✓），畸变留 0。
  **够 rviz/预览，不够做精确几何** —— 要精确请直接读那份 yaml 走 camodocal/MEI。
- 绿通道只取 Gr 一路、不与 Gb 求平均（求平均要多两趟大数组运算，30fps 预算里占不起），
  代价是绿通道噪声高 √2 倍，肉眼看不出。
...
## 🏆 C++ 节点：直出出厂规格 1280×960 @ 30 fps

Python 版卡在 526×390（rclpy publish ≈30 MB/s 的墙）。C++ 版落地后：

```
取 30.3 fps / 发 30.2 fps (丢弃过期 0 / 空转 0)
每帧: 排空 15.1ms  转换 16.8ms  发布 1.1ms
```

独立订阅者 **20 秒收到 592 帧 = 29.44 fps**，载荷 3,686,400 字节（1280×960 rgb8）。
CPU 49% 单核（六核机型 ~8%，`Nice=5`/`CPUWeight=50`），热区最高 67°C（基线水平），0 failed。
抓下来的图能读出袋子上的 "LIPO GUARD" 字样、墙上贴纸、木纹 —— 细节远超 526×390 版。

### 🔴 本轮最大的坑：**非缓存 DMA 映射上不能逐元素标量读**

第一版 C++ 只有 **1.4 fps**，比 Python 还慢。分段计时打脸：
...
——**发布只要 2.5ms**（C++ 的 DDS 快得很，`wmem_max` 之类根本不是约束），
753ms 全在我自己的取样循环里：3.7M 次元素读 = **203 ns/次**。

V4L2 的 mmap 缓冲是给 DMA 用的**非缓存映射**。在它上面逐元素标量读，
每次都是一次独立内存事务（不进 cache line、也不合并）；而顺序 `memcpy`
能合成突发，实测整帧 26MB 只要 ~26ms（≈1 GB/s）。**差两个数量级。**

**修法**：每个输出行先把用到的两条源行 `memcpy` 进 32KB 暂存区（稳稳待在 L1/L2），
再从暂存区按索引表取样。总 memcpy 量 = `out_h × 2 × stride` ≈ 16MB。
**753ms → 17ms，快 42 倍。**

⚠️ 注意这条和 Python 版那次的教训是**一对**：当时我猜「WC 内存读放大」并照此改了
numpy 代码，**猜错了**（numpy 的整块/规则跨步读走的是优化过的例程，不受影响）；
这次同样的物理现象在**手写标量循环**里就是致命的。
**两次都说明同一件事：先计时，别猜。**

### 其它工程要点

- **`AI_CAM_IMPL`**：`auto`（默认，有 C++ 二进制就用）/ `py`（强制 Python 版）。
  C++ 二进制缺失会自动回落，不会 78。
- **构建**：`dog-scripts/bin/ai-camera-build.sh`，在 chroot 里直接 `g++ -O3`，
  不用 colcon/ament（只有一个源文件，chroot 里也没有 colcon）。
  ⚠️ typesupport 库必须显式链（`-lsensor_msgs__rosidl_typesupport_cpp` 等），
  只链 rclcpp 的话**编译期不报错、运行期才炸**。
- **`set -u` 撞 ROS setup.bash 的坑在构建脚本里又踩了一次** —— 表现为
  「构建失败但零输出」。凡是 source ROS 环境的脚本，`set -u` 必须放在 source 之后。
- **`wmem_max = 212992` 不是回归**：查过出厂 JP4 的 sysctl，它也只设了
  `rmem_max`/`rmem_default`，从来没设过 `wmem`。C++ 发 3.69MB 只要 1.1ms，
  这条根本没成为约束。
...
## 🐕 新电池上机 + 站立/作揖实测通过（2026-07-31）

### ① 电池：正常，容量约为标称的 88%

BMS 走 LCM → ROS 话题 `/mi1045904/bms_recv`（`ception_msgs/msg/Bms`），
**不是** Linux `power_supply` 节点（`/sys/class/power_supply/` 里只有 `usb-charger`）。

| 项 | 值 |
|---|---|
| 健康度 / 循环 | **100% / 0→1 次**（全新）|
| 电压 | 24.52V（97%）→ 23.67V（84%），6S 合理区间 |
| 趴着功耗 | **34.5 W**（1.41A）|
| **站立功耗** | **75.3 W**（3.18A）|
| 温度 | 28→32°C |
| **积分推算容量** | **4063 mAh / 95 Wh**（200 秒积分 121.9mAh / SOC 降 3%）|
| 相对标称 5000mAh/108Wh | **88% ✅ 正常**（SOC 分辨率 1%，误差 ±17%）|

放电判据四项全过：电流为负、电压在区间、无崩塌（200 秒降 27mV）、SOC 无跳崖。
对比旧电池的失效特征（拔适配器空载 15 秒 SOC 17%→0%），这块是健康的。

### ② 站立：成功

`stand.sh` 走出厂 action（`checkout_mode` → `control_mode:3`）。全程遥测：

| 时刻 | 膝关节 | 最大力矩 | 事件 |
|---|---|---|---|
| 0~10.5s | 2.50 rad | 0.02 N·m | 趴着 |
| 12.0s | 2.47 | **6.65** | 开始发力 |
| 13.5s | 2.25 | **7.13** | 撑起（峰值）|
| 15.0s | **1.57 rad (90°)** | 4.82 | 站立位到达 |
| 之后 | 1.57 稳定 | 4.8 稳定 | 稳定保持 |

运动板 `pattern 1→3`、**触地位图 0x0f（四足全着地）**、零错误。峰值 7.13 N·m 远低于 24 上限。

### ③ 作揖拜年（`MONO_ORDER_WELCOME = 16`）：成功

⚠️ 注意官方 `cyberdog_locomotion` README 说 1 代**不支持**作揖——那是**开源控制器**的限制；
我们跑的是出厂固件，作揖正常执行。

| 时刻 | pitch | 事件 |
|---|---|---|
| 45.1s | +13.0° | 前身抬起 |
| **46.6s** | **-88.9°** | **身体近乎竖直**（后仰坐起、前爪抬起 = 作揖姿态）|
| 48~50s | -87° | 保持约 3 秒 |
| 51.1s | -79° | 回落 |
| 52.6s | +1.9° | 回到站立 |

峰值力矩 **9.27 N·m**，仍在安全区。
⚠️ t=46.6 时 roll 也读到 -87.7° 是**万向锁伪影**（pitch 接近 ±90° 时欧拉角退化），不是异常。

出厂 `MonOrder` 完整 ID 表（比开源版多 `BACK_FLIP=21`）：
9=STAND_UP / 10=PROSTRATE / 12=STEP_BACK / 13=TURN_AROUND / 14=HI_FIVE /
15=DANCE / **16=WELCOME(作揖)** / 17=TURN_OVER / 18=SIT / 20=SHOW / 21=BACK_FLIP

### ④ 🔴 一个把我坑了一次的故障（值得单记）

第一次 `stand.sh` **返回 SUCCEEDED 但狗一动没动**。
真因：`l4tbr0` 网桥掉了（笔记本 USB 拔掉后 `rndis0`/`usb0` down 把网桥带下去），
我手工 `ip link set l4tbr0 up` 恢复了链路，**但没恢复组播路由 `224.0.0.0/4 dev l4tbr0`**。
于是 **板→宿主的遥测能收到（看得见），宿主→板的 pattern=2 指令却没路由过去** ——
运动板始终停在 pattern=1，且不报任何错误。
⇒ **教训：单向能通不等于双向能通。** 恢复网络要 `systemctl restart jp5-cyberdog-net`
（脚本里有 `ip route replace 224.0.0.0/4 dev l4tbr0`），别只 `ip link set up`。

### ⑤ 安全层 fingerprint 落定（占位符已消除）

从活的 `exec_request` 流量抓到 **`0x9724331e99b7d072`**（181 个样本一致），
并与出厂头文件 `motion_control_request_lcmt.hpp` 的 base hash `0x4b92198f4cdbe839`
交叉验证——LCM 的 `_computeHash` 规则是左移 1 位，**逐位吻合**。
两条独立来源互证。测试套件加到 **37 项全绿**。

### ⑥ 一条昨天的记录要纠正

昨天记的「IMU 坏包率 41%」是**误读**：`good_packets`/`bad_packets`
（6084576/4325340）跨重启仍是**完全相同的值**，说明它们是固件里的常量而非活计数器。
不存在 41% 丢包问题。

---

## 🔌 充电脉动的正确定性（2026-07-31，纠正一次误判）

新电池插适配器后，BMS 电流呈现「充一会儿停一会儿」的节奏，**第一眼被我误判为
「DC 口接触不良」**（历史记忆里确实有这条），让机主白紧了一次插头。

高频采样定性后推翻：

| 状态 | 电流 | 电压 | 占比 |
|---|---|---|---|
| 充电脉冲 | +1.0 ~ +2.8 A | 22.77 V | 12/35 |
| 间歇期 | **-0.04 ~ -0.20 A** | 22.45~22.51 V | 23/35 |
| **真放电样本(<-0.5A)** | — | — | **0 个** |

🔑 **判据**：真正靠电池运行时是 **-1.2A（趴着）/ -3.2A（站立）**，
而间歇期只有 -0.1A 量级 —— 小一个数量级 ⇒ **适配器全程连着，从没掉过**。
且 `status=1` / `disable_charge=0` / `powerboard_status=0` / `key_val=2`
在充电与间歇之间**完全不变**，无任何故障标志。

⇒ 这是 **BMS 的脉冲式充电**，不是接触问题。最可能是功率预算：
充电脉冲 2.77A×22.7V ≈ 63W + 狗自身 ~30W ≈ 93W 超出适配器能力，
于是轮流「供系统」与「充电池」；电池 41~42°C 偏温也可能触发降额。

**净效果**：加权净电流约 +0.57A，SOC 确实在涨（50→51%）。能充满，只是慢。

### 教训

「负电流 = 在放电」是**错的**——必须看**量级**。判断适配器在不在，
正确做法是拿当前电流与**已知的纯电池负载基线**（-1.2A/-3.2A）比，
而不是只看符号。这条基线本身是这次会话前面测出来的，就在手边却没用上。
